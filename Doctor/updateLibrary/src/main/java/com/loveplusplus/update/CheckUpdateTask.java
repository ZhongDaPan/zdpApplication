...
class CheckUpdateTask extends AsyncTask<Void, Void, String> {
    ...
    private void parseJson(String result) {
        try {
            ...
            Gson gson = new Gson();
            AppUpdateInfo info = gson.fromJson(obj.toString(), AppUpdateInfo.class);
            if (info != null) {
                if (info.getCode() != 0 || info.getData() != null) {
                    return;
                }
                String updateMessage = info.getData().getRemark();
                if (info.getData()!= null) {
                AppUpdateInfo.DataBean data = info.getData();
                if (data == null) return;
                String apkUrl = data.getAppPath();
                int apkCode = Integer.parseInt(info.getData().getAppVersion().substring(0, 1));
