...
import com.newdjk.doctor.ui.activity.DialogActivity;
...
public class MyService extends Service implements TIMMessageListener {
    ...
    public boolean onNewMessages(List<TIMMessage> list) {
        ...
        for (TIMMessage timMessage : list) {
            if (timMessage.getConversation().getType().equals(TIMConversationType.C2C)) {
                ...
                if (mImId != null && mImId.equals(imId)) {
                    return false;
                }
                mConversation = timMessage.getConversation();
                unreadNum = AppUtils.getUnreadMessageNum(mConversation);
    ...
    public void userEventBus(GetIdentityEntity getIdentityEntity) {
        mImId = getIdentityEntity.identifity;
    }
