        ...
        mMyOkhttp.get().url(url).tag(this).enqueue(new GsonResponseHandler<ResponseEntity<DoctorInfoByIdEntity>>() {
            ...
            public void onSuccess(int statusCode, final ResponseEntity<DoctorInfoByIdEntity> entity) {
                if (entity.getCode() == 0) {
                    ...
                    PersonalDataActivity.activity.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            int sexType = SpUtils.getInt(Contants.Sex, 0);
                            String sex = "";
                            ...
                            int status = SpUtils.getInt(Contants.Status, 0);
                            switch (status) {
                                case 0:
                                    mFunPhone.setRightText("未认证");
                                    break;
                            ...
                            mFunName.setRightText(entity.getData().getDrName());
                            mFunGender.setRightText(sex);

                            //   funYuE.setRightText();
                            mFunBirth.setRightImage(getResources().getDrawable(R.mipmap.doctor_home_card));
                            mFunHospitol.setRightText(entity.getData().getHospitalName());
                            mFunDepartment.setRightText(entity.getData().getDepartmentName());
                            mFunTitle.setRightText(entity.getData().getTreatMent());
                            title.setRightText(entity.getData().getPositionName());
                            mFunGoodAt.setRightText(entity.getData().getDoctorSkill());
                            mFunIntroduction.setRightText(entity.getData().getDescription());
                            Log.i("PersonalDataActivity", "path=" + entity.getData().getPicturePath());
                            Glide.with(MyApplication.getContext())
                                    .load(entity.getData().getPicturePath())
