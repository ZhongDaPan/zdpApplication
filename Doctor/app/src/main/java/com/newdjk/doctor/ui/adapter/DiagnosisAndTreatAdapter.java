...
public class DiagnosisAndTreatAdapter extends BaseQuickAdapter<DoctorMedicalRecordsEntity.ReturnDataBean, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, final DoctorMedicalRecordsEntity.ReturnDataBean item) {
        ...
        try {
            ...
            if (mType == 2) {
                helper.getView(R.id.deal_status).setVisibility(View.VISIBLE);
                if (item.getMedicalStatus() == 4) {
                    helper.setText(R.id.deal_status, "已完成");
                } else if (item.getMedicalStatus() == 5) {
                    helper.setText(R.id.deal_status, "已退单");
                }

            } else {
                helper.getView(R.id.deal_status).setVisibility(View.GONE);
            }
        } catch (Exception e) {
